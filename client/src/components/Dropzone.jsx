...
const DropzoneComponent = ({ onFileSelect }) => {
  ...
  const onDrop = useCallback((acceptedFiles) => {
    ...
    onFileSelect(selectedFile); // Pass selected file object to parent component
  }, [onFileSelect]);
  ...
  return (
    <div {...getRootProps()} className={`mb-4 border-2 border-dashed p-4 rounded-lg ${isDragActive ? 'border-blue-500' : 'border-gray-300'}`}>
      ...
      <div className="flex items-center justify-center">
        {!file && 
          <div className="w-160 h-16 mb-4 text-gray-500 flex flex-col items-center justify-center">
            <h1 className="mb-2">Click or drag to upload image</h1>
        ...
        {file && (
          <div>
            <img src={file.preview} alt="Preview" className="w-full h-[200px] md:h-[200px] object-contain" />
          </div>
        )}
      </div>
    </div>
